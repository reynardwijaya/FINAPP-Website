...
@props(['name', 'label' => null, 'hint' => null, 'optional' => false, 'prefix' => null, 'value' => null, 'type' => 'text', 'wrapperClass' => '', 'bag' => null, 'useOld' => true])
@php
    $id = $attributes->get('id', $name);
    $hasError = ($bag ?? $errors)->has($name);
...
@endphp
<x-field :label="$label" :name="$name" :for="$id" :hint="$hint" :optional="$optional" :class="$wrapperClass" :bag="$bag">
    <div class="relative">
        @if ($prefix)
        ...
        <input type="{{ $type }}" name="{{ $name }}" id="{{ $id }}"
               @if ($type !== 'password' && $type !== 'file') value="{{ $useOld ? old($name, $value) : $value }}" @endif
